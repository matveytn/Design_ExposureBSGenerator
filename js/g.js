(function () {
	...
	window.IpsumGenerator = {
		Models: {}, Collections: {}, Views: {}, getRandomNumber: function (min, max) {
	...
	IpsumGenerator.Models.Terms = Backbone.Model.extend({
		initialize: function () {
			...
			var sentence = '', wordCount = IpsumGenerator.getRandomNumber(7, 14);
			...
			var paragraph = '', sentenceCount = IpsumGenerator.getRandomNumber(3, 5);
	...
	IpsumGenerator.Views.IpsumText = Backbone.View.extend({
		...
		bindBreakpoints: function () {
			var $window = $(window), $creators = $('#creators'), $socialLinks = $('#socialLinks');
			$window.setBreakpoints({distinct: true, breakpoints: [961]});
		...
		render: function (evt) {
			evt.preventDefault();
			var that = this, $startupIpsum = this.$el.find('#startup-ipsum'), content = {paragraphs: this.generateIpsum()};
		...
		renderHeaderIcon: function () {
			var imgNum = IpsumGenerator.getRandomNumber(1, 4), url = 'images/badge-' + imgNum + '.png';
